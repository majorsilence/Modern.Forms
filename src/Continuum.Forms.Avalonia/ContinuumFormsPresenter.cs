...
{
    /// <summary>
    /// An Avalonia control that embeds a Continuum.Forms scene inside an existing Avalonia window or
    /// panel. Drop it into your Avalonia visual tree (XAML or code) and assign <see cref="Content"/> a
    /// Continuum.Forms control tree; it renders into the host window's surface and forwards Avalonia
    /// input into the Continuum.Forms pipeline. No top-level OS window is created.
    ///
    /// Continuum.Forms is rendered on the UI thread into a <see cref="WriteableBitmap"/> framebuffer
    ...
    /// Continuum scene via <see cref="NativeControlHost"/> (the airspace interop model).
    /// </summary>
    public class ContinuumFormsPresenter : Canvas, IWindowBackend, INativeControlHostBackend
    {
        ...
        // The Continuum scene is drawn into this Image (a hit-test-invisible bottom child) since Panel.Render
        ...
        public ContinuumFormsPresenter ()
        {
            ...
            // embedded scene even over areas the Continuum content leaves transparent.
        ...
        /// theme (light/dark + accent). Defaults to true. Because Continuum's Theme is global, the last
        ...
        protected override void OnAttachedToVisualTree (VisualTreeAttachmentEventArgs e)
        {
            ...
            if (SyncTheme)
                ContinuumFormsTheme.FollowHost ();
        ...
        // Renders the Continuum.Forms scene into the framebuffer on the UI thread, then asks Avalonia to
        ...
        // ── Input forwarding (Avalonia → Continuum.Forms; positions scaled to physical pixels) ───────
        ...
        // Cursor property is Avalonia.Input.Cursor (aliased AvCursor below to avoid the Continuum.Forms.Cursor collision).
